...
# ********* CANCEL SCAN BUTTON *********************
    ...
    def scan_btn_pressed(self):
        ...
        self.run_scan(target, port_bot, port_top)
    ...
    def run_scan(self, tar, bot, top):
        self.Output_display_box.append('Starting Scan.....')
        self.port_bot = bot
        self.port_top = top
        self.tar = tar
        print("before p", self.tar, " ", self.port_top, " ", self.port_bot)
        p = nmap_port_scanner.Port_Scan(self.tar, self.port_bot, self.port_top)
        print("after p")
        p.scanner()
        print("after scanner")
        for host in p.nm.all_hosts():
            self.Output_display_box.append('----------------------------------------------------')
            self.Output_display_box.append('Host : %s (%s)' % (host, p.nm[host].hostname()))
            self.Output_display_box.append('State : %s' % p.nm[host].state())
            for proto in p.nm[host].all_protocols():
                self.Output_display_box.append('----------')
                self.Output_display_box.append('Protocol : %s' % proto)

                lport = p.nm[host][proto].keys()

                for port in lport:
                    self.Output_display_box.append('port : %s\tState: %s\t Service: %s\t Version: %s %s'
